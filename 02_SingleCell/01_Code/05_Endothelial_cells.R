...
path_code <- "E:/WZH/01_Single/GEO/09_Endothelial_cells/Code/"
path_Endothelial_cells <- "E:/WZH/01_Single/GEO/09_Endothelial_cells/Data/"
setwd(Path_Data)

load("All_Cell_Cluster.RData")
load("cluster8_8.markers.RData")
...
# 获取内皮细胞位置
#####################
Endothelial_cells_position <- which(All_Cell_Cluster$Cluster_number == 8)

# 获取内皮细胞样本,并去除其他细胞样本
Endothelial_Sample <- All_Cell_Cluster$Cell_id[Endothelial_cells_position]
Endothelial_Sample <- Endothelial_Sample[-which(All_Cell_Cluster$Cell_Type[Endothelial_cells_position] != "Endothelial_cells")]
...
# 获取所有差异基因
diff8 <- rownames(cluster8_8.markers)
# 获取上调和下调基因
diff8_up <- diff8[which(cluster8_8.markers$avg_log2FC > 0.5)]
diff8_0 <- diff8[which(cluster8_8.markers$avg_log2FC == 0)]
diff8_down <- diff8[which(cluster8_8.markers$avg_log2FC < -0.5)]

# 获取内皮细胞以基差异基因的单细胞表达矩阵
diff8_gene_position <- which((all_gene %in% diff8) == TRUE)
Endothelial_Sample_position <- which(all_sample %in% Endothelial_Sample)

Endothelial_assays_data <- pbmc_all_assays_data[diff8_gene_position, Endothelial_Sample_position]

# 计算差异基因之间的相关性
diff8_gene_corAndPvalue <- corAndPvalue(t(Endothelial_assays_data))
